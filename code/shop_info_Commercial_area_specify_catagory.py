...
from scrap_all_menus import read_scrap_all_menus_Commercial_area,read_specify_catagory_all_menus_Commercial_area
...
def get_Shop_Info(driver,shop_catagory,happen_except,cur_index,shoplist_or_eachShop,city,shop_district):
    ...
    global list_svg_y
    global dict_css_x_y
    code_city = {'bejing':'北京市','shanghai':'上海市','nanjing':'南京市','chengdu':'成都市','chongqing':'重庆市','wuhan':'武汉'}
    shop_info = cShopInfo()
...
if __name__ == '__main__':
    ...
    filename = 'Commercial_area_food_urllist_' + city + '.txt'
    specify_food_file = city + '_' + specify_food + '.txt'
    if os.path.exists(specify_food_file) != True:
        fp = open(specify_food_file, 'w')
        fp.close()
